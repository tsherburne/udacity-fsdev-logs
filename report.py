...
c.execute("select a.title, count(*) as num from log l, articles a \
    where l.path = concat('/article/', a.slug) and l.status = '200 OK' and l.path != '/' \
...
c.execute("select name, sum(num) from \
    (select au.name, a.title, count(*) as num from log l, articles a, authors au \
        where l.path = concat('/article/', a.slug) and a.author = au.id and l.status = '200 OK' \
            and l.path != '/' group by au.name, a.title order by num desc) as popular \
...
c.execute("select l.date, l.total, e.total, \
                round((cast(e.total as numeric)/ cast(l.total as numeric) * 100), 2) as percent \
        from daily_logs l, daily_errors e where l.date = e.date and \
...
for report_item in results:
    print(report_item[0].strftime('%b %d %Y') + " -- " + str(report_item[3]) + "% errors")
